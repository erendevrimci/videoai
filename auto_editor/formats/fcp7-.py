...
def media_def(
    ...
    # Set name from source path, handling different object types
    if hasattr(src, 'path') and hasattr(src.path, 'stem'):
        ET.SubElement(filedef, "name").text = src.path.stem
    else:
        from pathlib import Path
        ET.SubElement(filedef, "name").text = Path(str(src)).stem
    ...
    if hasattr(src, 'videos') and len(src.videos) > 0:
        ...
        # For Path objects or other sources without videos attribute, create a default video section
        videodef = ET.SubElement(mediadef, "video")
        vschar = ET.SubElement(videodef, "samplecharacteristics")
        rate = ET.SubElement(vschar, "rate")
        ET.SubElement(rate, "timebase").text = f"{tb}"
    ...
    # Handle FileInfo objects with audios attribute
    if hasattr(src, 'audios'):
        ...
        # For Path objects or other sources without audios attribute, create a default audio section
        audiodef = ET.SubElement(mediadef, "audio")
        aschar = ET.SubElement(audiodef, "samplecharacteristics")
        ET.SubElement(aschar, "depth").text = DEPTH
        ET.SubElement(aschar, "samplerate").text = f"{tl.sr}"
        ET.SubElement(audiodef, "channelcount").text = "2"  # Default to stereo
...
def premiere_write_audio(audio: Element, make_filedef, src, tl: v3) -> None:
    ...
    for aclips in tl.a:
        for channelcount in range(0, 2):  # Because "stereo" is hardcoded.
            ...
            for j, aclip in enumerate(aclips):
                ...
                # Determine clip number based on video presence
                has_videos = hasattr(clip_src, 'videos') and clip_src.videos
                if not has_videos:
                    clip_item_num = j + 1
                else:
                ...
                # Set name from source path, handling different object types
                if hasattr(clip_src, 'path') and hasattr(clip_src.path, 'stem'):
                    ET.SubElement(clipitem, "name").text = clip_src.path.stem
                else:
                    from pathlib import Path
                    ET.SubElement(clipitem, "name").text = Path(str(clip_src)).stem
                ET.SubElement(clipitem, "enabled").text = "TRUE"
                ET.SubElement(clipitem, "start").text = _start
                ET.SubElement(clipitem, "end").text = _end
                ET.SubElement(clipitem, "in").text = _in
...
def fcp7_write_xml(name: str, output: str, resolve: bool, tl: v3) -> None:
    ...
    if len(tl.v) > 0 and len(tl.v[0]) > 0:
        ...
        for j, clip in enumerate(tl.v[0]):
            ...
            # Set name from source path, handling different object types
            if hasattr(clip.src, 'path') and hasattr(clip.src.path, 'stem'):
                ET.SubElement(clipitem, "name").text = clip.src.path.stem
            else:
                from pathlib import Path
                ET.SubElement(clipitem, "name").text = Path(str(clip.src)).stem
            ET.SubElement(clipitem, "enabled").text = "TRUE"
            ET.SubElement(clipitem, "start").text = _start
            ET.SubElement(clipitem, "end").text = _end
            ET.SubElement(clipitem, "in").text = _in
            ...
            # Get number of audio channels, defaulting to 2 (stereo) if not available
            num_audio_channels = 2  # Default to stereo
            if hasattr(src, 'audios'):
                num_audio_channels = sum(aud.channels for aud in src.audios)
            
            for i in range(1 + num_audio_channels):  # Add 1 for video track
                link = ET.SubElement(clipitem, "link")
                ET.SubElement(
                    link, "linkclipref"
