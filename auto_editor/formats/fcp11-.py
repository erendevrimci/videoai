...
def fcp11_write_xml(
    ...
    if version == 11:
        ...
        ver_str = "1.1"  # Version 1.1 is most compatible for old FCP
    else:
    ...
    for i, one_src in enumerate(tl.unique_sources()):
        ...
        # Only add colorSpace for version 10+ as older DTDs explicitly don't support it
        # Final Cut Pro version 10.4.6+ supports colorSpace attribute
        if version >= 10:
        ...
        # Use strictly DTD-compliant structures based on version
        if version >= 10:
            ...
            # For older versions, use only metadata without src attribute
            # Version 5 DTD strictly requires metadata instead of media-rep
    ...
    # Create sequence with appropriate attributes based on version
    seq_props = {
        "format": "r1",
        "tcStart": "0s",
    ...
    # Only add tcFormat for versions that support it (6+)
    if version >= 6:
        seq_props["tcFormat"] = "NDF"
        
    sequence = SubElement(proj, "sequence", seq_props)
    spine = SubElement(sequence, "spine")

    def make_clip(ref: str, clip: TlVideo | TlAudio) -> None:
        # Basic properties needed for all clip types
        clip_properties = {
            "name": proj_name,
            "ref": ref,
